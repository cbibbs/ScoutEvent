...
- [x] T1.1 Needs Review: paginated query (oldest-first, `range`,
      `count: "exact"`), "Load more" footer with accurate
      "Showing X of Y" (design §1).
- [x] T1.2 Slideshow: paginated query (`status=approved`,
      `in_slideshow=true`, oldest-first to match TV playback order),
      same "Load more" pattern (design §1).
- [x] T1.3 Library: paginated query with status filter, in-slideshow
      filter, uploader-name search, sort, same "Load more" pattern
      (design §1).
- [x] T1.4 Revise the realtime handler and 30s poll fallback per design
...
- [x] T2.1 Library toolbar: search input, status filter, in-slideshow
      filter, sort control — each change resets to page 1.
- [x] T2.2 Lazy-load grid images (`loading="lazy"`) per design §5's
      free mitigation.

## Phase 3 — Bulk actions (US-13, US-15)

- [x] T3.1 Selection state (per section) + checkbox affordance on each
      card — visible by default in Needs Review and Slideshow, behind a
      "Select" toggle in Library (design §2).
- [x] T3.2 Bulk action bar: Approve/Reject selected (Needs Review);
...
- [x] T4.1 Focused single-photo review view: Approve/Reject/Skip,
...
- [x] T5b.1 Phone-width layout for Manage Event (same component, one
...
- [ ] T5.2 Live end-to-end check against the real Supabase project with
      ...
      action, and one-at-a-time review. Guest-facing screens (landing,
      guest upload, slideshow) verified live in-browser against
      production data; the organizer screens (Manage Event, Review one
      at a time) still need an authenticated in-browser pass — no login
      session was available in the sandbox that implemented this.
